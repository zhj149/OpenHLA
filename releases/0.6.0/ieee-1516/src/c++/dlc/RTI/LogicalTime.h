...
namespace rti1516
{
  ...
  public:
    ...
    virtual LogicalTime& operator=(LogicalTime const& value)
      throw(InvalidLogicalTime) = 0;

    virtual LogicalTime& operator+=(LogicalTimeInterval const& addend)
      throw(IllegalTimeArithmetic, InvalidLogicalTimeInterval) = 0;

    virtual LogicalTime& operator-=(LogicalTimeInterval const& subtrahend)
      throw(IllegalTimeArithmetic, InvalidLogicalTimeInterval) = 0;

    virtual bool operator>(LogicalTime const& value) const
      throw(InvalidLogicalTime) = 0;
  ...
  std::wostream& RTI_EXPORT operator<<(std::wostream& wos, LogicalTime const& logicalTime);
}
