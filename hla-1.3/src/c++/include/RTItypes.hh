...
#define MAX_FEDERATION            65535
#define MAX_FEDERATE              65535
#define MAX_NAME_LENGTH           65535
#define MAX_SPACES                65535
#define MAX_OBJECT_CLASSES        65535
#define MAX_INTERACTION_CLASSES   65535
#define MAX_ATTRIBUTES_PER_CLASS  65535
#define MAX_PARAMETERS_PER_CLASS  65535
#define MAX_DIMENSIONS_PER_SPACE  65535
#define DEFAULT_SPACE_NAME        "defaultSpace"
#define MAX_USER_TAG_LENGTH       65535
#define RTI_VERSION               "1.0"
#define MAX_EXTENT                4294967295
#define MIN_EXTENT                0
...
class RTI_EXPORT AttributeHandleValuePairSet
{
  //
  // -----------------------------------------------------------------------------------------------------------------------
  //  Instances of class HandleValuePairSet are the containers used to pass object attribute values and interaction
  //  parameter values between the Fedrate and the RTI. These containers hold sets of attribute/parameter values indexed by
  //  their attribute/parameter handle. Instances of this class are provided to the RTI in the Update Attribute Values and
  //  Send Interaction service invocations. Instances of this class are provided to the Federate in the Reflect Attribute
  //  Values and Receive Interaction service invocations. When instances of HandleValuePairSet are provided to the Federate
  //  by the RTI, the memory used to store attribute/parameter values is valid for use by the federate only within the
  //  scope of the Reflect Attribute Values or Receive Interaction service invocation. Symmetrically, for instances of
  //  HandleValuePairSet provided by the Federate to the RTI, the memory used to store attribute/parameter values is valid
  //  for use by the RTI only within the scope of the Update Attribute Values or Send Interaction service invocation.
  // -----------------------------------------------------------------------------------------------------------------------
  //
public:
  ...
  virtual inline ULong start() const = 0;
  virtual inline ULong valid(ULong i) const = 0;
  virtual inline ULong next(ULong i) const = 0;
};
...
public:
  ...
  virtual ULong getRangeUpperBound(ExtentIndex extentIndex, DimensionHandle dimensionHandle) const
    throw(ArrayIndexOutOfBounds) = 0;
  virtual void setRangeLowerBound(ExtentIndex extentIndex, DimensionHandle dimensionHandle, ULong theLowerBound)
    throw(ArrayIndexOutOfBounds) = 0;
  virtual void setRangeUpperBound(ExtentIndex extentIndex, DimensionHandle dimensionHandle, ULong theUpperBound)
    throw(ArrayIndexOutOfBounds) = 0;
  virtual SpaceHandle getSpaceHandle() const
    throw() = 0;
  virtual ULong getNumberOfExtents() const
    throw() = 0;
  virtual ULong getRangeLowerBoundNotificationLimit(ExtentIndex extentIndex, DimensionHandle dimensionHandle) const
    throw(ArrayIndexOutOfBounds) = 0;
  virtual ULong getRangeUpperBoundNotificationLimit(ExtentIndex extentIndex, DimensionHandle dimensionHandle) const
    throw(ArrayIndexOutOfBounds) = 0;
};
